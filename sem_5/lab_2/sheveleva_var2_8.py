...
def dF_direct(Z):
    h = 1e-6
    return (F_direct(Z + h) - F_direct(Z - h)) / (2 * h)

#найти вторую производную
#у первой и второй производной должен быть постоянный знак на интервале

def descartes_localization(a, b, steps=1000):
    x_values = np.linspace(a, b, steps)
    for i in range(steps - 1):
        if F_direct(x_values[i]) * F_direct(x_values[i+1]) < 0:
...
# Поиск корня F(Z) = 0
interval = descartes_localization(0.1, 1.0)
if interval:
    a, b = interval
    print(f"\nКорень локализован в интервале [{a:.6f}, {b:.6f}]")
    Z_solution = newton_method((a + b) / 2)
    if Z_solution:
        print(f"Метод Ньютона: Z = {Z_solution:.6f}, F(Z) = {F_direct(Z_solution):.3e}")
        if lowest_border <= abs(Z_solution) <= highest_border:
            print("Корень в локализации")
        else:
            print("Корень вне локализации")
    else:
        print("Метод Ньютона не сошелся")
else:
    print("Корень не найден в интервале")
